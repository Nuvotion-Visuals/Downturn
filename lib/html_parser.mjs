 ...
 *
 * Extended with: querySelector, querySelectorAll, outerHTML, cloneNode, nodeValue/data
 * Converted to ESM for urltomarkdown zero-dependency rewrite.
 */
  ...
  function decodeHTML(str) {
    return str.replace(/&([a-zA-Z]+);/g, function(match, tag) {
      return entityTable[tag] !== undefined ? entityTable[tag] : match;
  ...
  JSDOMParser.prototype = {
    ...
    discardTo: function (str) {
      var found = this.html.indexOf(str, this.currentChar);
      if (found === -1) {
        this.currentChar = this.html.length;
        return;
      }
      this.currentChar = found + str.length;
    },
    ...
    readNode: function () {
      ...
      if (!closed && localName in voidElems) {
        // Void element without self-closing slash — consume optional closing tag
        this.match("</" + node._matchingTag + ">");
      } else if (!closed) {
        this.readChildren(node);
        var closingTag = "</" + node._matchingTag + ">";
        if (!this.match(closingTag)) {
          // Forgiving: consume mismatched closing tag if present (e.g., </ol> closing a <ul>)
          if (this.html[this.currentChar] === '<' && this.html[this.currentChar + 1] === '/') {
            this.discardTo(">");
          }
        }
      }
  ...
  // --- Extensions for urltomarkdown ---

  // Helper: find element by ID recursively (for Element nodes that lack getElementById)
  ...
  // querySelectorAll helper — handles tag names, #id, and comma-separated selectors
  // Returns a plain array (no _isLiveNodeList flag) to satisfy Readability's safety check
  function qsa(node, selector) {
    ...
    for (var p = 0; p < parts.length; p++) {
      ...
      if (sel.startsWith('#')) {
        var byId = node.getElementById
          ? node.getElementById(sel.slice(1))
          : findById(node, sel.slice(1));
        if (byId) results.push(byId);
      } else if (sel.startsWith('.')) {
        // class selector — walk all elements
        ...
        var byTag = node.getElementsByTagName(sel);
        for (var j = 0; j < byTag.length; j++) {
          results.push(byTag[j]);
        }
      }
    }
    return results;
  ...
  // Add querySelector/querySelectorAll to Document
  Document.prototype.querySelector = function(selector) {
    return qsa(this, selector)[0] || null;
  };
  Document.prototype.querySelectorAll = function(selector) {
    return qsa(this, selector);
  };

  // Add querySelector/querySelectorAll to Element
  ...
  // Add getElementById to Element (for Turndown's RootNode which calls doc.getElementById)
  Element.prototype.getElementById = function(id) {
    return findById(this, id);
  };

  // Add outerHTML getter to Element
  Object.defineProperty(Element.prototype, 'outerHTML', {
    get: function() {
      ...
      for (var i = 0; i < this.attributes.length; i++) {
        ...
        var val = attr.getEncodedValue();
        var quote = val.indexOf('"') === -1 ? '"' : "'";
        html += ' ' + attr.name + '=' + quote + val + quote;
      }
      if (tag in voidElems && !this.childNodes.length) {
        return html + '/>';
      }
  ...
  Node.prototype.cloneNode = function(deep) {
    ...
    var clone = new Element(this.localName);
    for (var i = 0; i < this.attributes.length; i++) {
      clone.attributes.push(new Attribute(this.attributes[i].name, this.attributes[i].value));
    }
    if (deep) {
  ...
  // Add nodeValue/data to Text nodes (Turndown reads these)
  ...
  // --- JSDOM wrapper class (matches jsdom's API) ---
  ...
  export { JSDOMParser, Node, Element, Text, Comment, Document, Attribute, JSDOM };
