...
I've written a couple of posts in the past few months but they were all for [the blog at work](http://blog.ioactive.com/search/label/Andrew%20Zonenberg) so I figured I'm long overdue for one on Silicon Exposed.

So what's a GreenPak?
---------------------

Silego Technology is a fabless semiconductor company located in the SF Bay area, which makes (among other things) a line of programmable logic devices known as GreenPak. Their [5th generation parts](http://www.silego.com/products/greenpak5.html) were just announced, but I started this project before that happened so I'm still targeting the [4th generation](http://www.silego.com/products/greenpak4.html).

GreenPak devices are kind of like itty bitty [PSoCs](http://www.cypress.com/products/32-bit-arm-cortex-m-psoc) - they have a mixed signal fabric with an ADC, DACs, comparators, voltage references, plus a digital LUT/FF fabric and some typical digital MCU peripherals like counters and oscillators (but no CPU).
...
Write your HDL, compile with Yosys, P&R with gp4par, and import the bitstream into GreenPak Designer to program the target device. The most current gp4par manual is included in LaTeX source form in the source tree and is automatically built as part of the compile process. If you're just browsing, there's a [relatively recent PDF version](http://thanatos.virtual.antikernel.net/unlisted/gp4-hdl.pdf) on my web server.

If you'd like to see the Verilog that produced the nightmare of a schematic I showed above, [here it is](https://github.com/azonenberg/openfpga/blob/master/tests/greenpak4/Blinky/Blinky.v).
...
They've even [offered me free hardware](https://twitter.com/SilegoTech/status/717018987771469824) to help me add support for their latest product family, although I plan to get GreenPak4 support to a more stable state before taking them up on the offer.
