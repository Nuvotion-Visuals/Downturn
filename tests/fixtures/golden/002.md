...
The [Fetch specification](https://fetch.spec.whatwg.org%29, which defines the API, nails down the semantics of a user agent fetching a resource. This, combined with ServiceWorkers, is an attempt to:
...
in the [specification](https://fetch.spec.whatwg.org%29. For example, you may not append or set a “request” guarded Headers’ “Content-Length” header. Similarly, inserting “Set-Cookie” into a Response header is not allowed so that ServiceWorkers may not set cookies via synthesized Responses.
...
*   [FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData) – currently not supported by either Gecko or Blink. Firefox expects to ship this in version 39 along with the rest of Fetch.
...
This decision allows easing the transition to an eventual [stream-based](https://streams.spec.whatwg.org%29 Fetch API. The intention is to let applications consume data as it arrives, allowing for JavaScript to deal with larger files like videos, and perform things like compression and editing on the fly.
...
_The author would like to thank Andrea Marchesini, Anne van Kesteren and Ben  
